<script lang="ts">
  ...
  let { item, progress = 0, playing = false }: { item: GalleryItem; progress?: number; playing?: boolean } = $props();
  ...
  const frameRect = $derived({
    x: .5 + (item.nest.x - .5) * Math.max(1, sourceAspect),
    y: .5 + (item.nest.y - .5) * Math.max(1, 1 / sourceAspect),
    w: item.nest.w * Math.max(1, sourceAspect),
    h: item.nest.h * Math.max(1, 1 / sourceAspect)
  });
  ...
  $effect(() => {
    ...
    loadGalleryScene(selected).then(loaded => {
      ...
      if (!cancelled) error = 'This image could not load. Try again.';
    });
  ...
  const smooth = (v: number) => { const n = Math.min(1, Math.max(0, v)); return n * n * (3 - 2 * n); };
</script>
...
<div class="artwork-stage" bind:this={host} data-ready={ready} data-progress={visual.toFixed(2)} data-renderer={cpu ? 'cpu' : 'webgl'} aria-label={`${item.title}, ${progress < .5 ? 'original image' : progress < 1.5 ? 'Droste repetition' : 'tententoon spiral'}`} role="group">
  {#key `${item.src}:${retry}`}
    <img class="source" src={item.src} alt={item.alt} decoding="async" />
  {/key}
  <div class="transformed" style:opacity={ready ? smooth(visual * 1.5) : 0} style:clip-path={`circle(${smooth(visual) * 145}% at ${Math.max(0, Math.min(1, frameRect.x + frameRect.w / 2)) * 100}% ${Math.max(0, Math.min(1, frameRect.y + frameRect.h / 2)) * 100}%)`}>

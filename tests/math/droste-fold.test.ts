...
/**
 * The fold's fundamental domain follows ctx.shapeMorph:
 *   1 (or omitted) — working rectangle [0, W-1]×[0, H-1]
 *   0             — ellipse E₀ inscribed in it (first hit lands in the
...
function ctx(shapeMorph?: number): DrosteCtx {
  return {
    cx: 50,
    cy: 50,
    logS: Math.log(2),
...
describe('sampleDroste fold domain', () => {
  ...
  it('rect mode samples the corner region directly', () => {
    ...
    expect(sampleDroste(pixels, ctx(1), px, py, out)).toBe(true);
    expect(out[0]).toBeCloseTo(px, 0); // red ≈ x → landed at the point itself
    expect(out[1]).toBeCloseTo(py, 0);
  });

  it('shapeMorph omitted behaves as rect (backward compat)', () => {
    const out: [number, number, number, number] = [0, 0, 0, 0];
  ...
  it('ellipse mode folds the corner point one step inward, into the ring', () => {
    const out: [number, number, number, number] = [0, 0, 0, 0];
    expect(sampleDroste(pixels, ctx(0), px, py, out)).toBe(true);
  ...
  it('ellipse mode folds an inner point outward into the fundamental annulus', () => {
    ...
    expect(sampleDroste(pixels, ctx(0), ix, iy, out)).toBe(true);
